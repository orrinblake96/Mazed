...
namespace GRIDCITY
{
    public class RouteTile : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                if (navCityManager.navMeshReady && mySlope == 0)
                {
                    int rando = Random.Range(0, 1000);
                    if (rando < 3)
                    {
                        Instantiate(navCityManager.agentPrefab, transform);
                    }
                }
            }
        }
...
//                    }
            ...
            {
                int corrector = (maxLevel-recursionCount) / 20;  //biasing randomization in the beginning to avoid extinction
